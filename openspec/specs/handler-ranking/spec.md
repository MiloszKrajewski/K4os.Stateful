# Spec: Handler Ranking

## Purpose

Defines how the state machine ranks registered transition rules when dispatching an event. Rules are ranked by type specificity (state then event), class-vs-interface kind, presence of a guard predicate, and declaration order as a stable tie-break.
